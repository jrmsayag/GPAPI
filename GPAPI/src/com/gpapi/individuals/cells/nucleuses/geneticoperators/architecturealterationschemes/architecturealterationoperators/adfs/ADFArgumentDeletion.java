...
public final class ADFArgumentDeletion implements ArchitectureAlterationOperatorInterface {
	...
	public final boolean architectureAlteration(Nucleus nucleus) {
		...
		for(AbstractNode argProxy : argProxiesToBeReplaced){
			...
			if(argProxy != adf.getRoot())
				argProxy.getParent().replaceArg(argProxy, replacingNode);
			else
				adf.replaceBranchRoot(argProxy, replacingNode);
		}
				...
				for(AbstractNode node : geneticOperator.getDescendantNodes(root)){
					if(node instanceof ADFProxy){
						...
						if(adfProxy.getId() == adf.getId()){
							adfProxy.getArgs().remove(deletedArgId);
							consolidate = true;
						}
					}
				}
	...
	@Override
	public final ADFArgumentDeletion copy() {
		return new ADFArgumentDeletion();
	}
}
