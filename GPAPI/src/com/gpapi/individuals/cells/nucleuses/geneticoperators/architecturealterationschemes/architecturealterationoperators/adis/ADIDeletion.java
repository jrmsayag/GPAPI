...
public final class ADIDeletion implements ArchitectureAlterationOperatorInterface {
	...
	public final boolean architectureAlteration(Nucleus nucleus) {
		...
		int deletedAdiId = ThreadLocalRandom.current().nextInt(adis.size());
		...
		if(nucleus.isHierarchicalAdis())
			referencingModules.addAll(adis.subList(deletedAdiId + 1, adis.size()));
		
		for(AbstractModule module : referencingModules){
			boolean consolidate = false;
			for(AbstractNode root : module.getBranchRoots()){
				if(adaptBranch(module, root, deletedAdiId, geneticOperator))
					consolidate = true;
			}
			if(consolidate)
				module.consolidate();
		...
		for(int i = deletedAdiId + 1; i < adis.size(); i++)
			adis.get(i).setId(i-1);
		adis.remove(deletedAdiId);
		
		return true;
	}
	
	private final boolean adaptBranch(AbstractModule module, AbstractNode root, int deletedAdiId, GeneticOperatorInterface geneticOperator){
		List<AbstractNode> adiProxiesToBeReplaced = new ArrayList<AbstractNode>();
		...
		for(AbstractNode node : geneticOperator.getDescendantNodes(root)){
			if(node instanceof ADIProxy && ((ADIProxy) node).getId() == deletedAdiId)
				adiProxiesToBeReplaced.add(node);
			else if(node.getArgs().isEmpty())
				potentialReplacingNodes.add(node);
		}
		
		for(AbstractNode adiProxy : adiProxiesToBeReplaced){
			AbstractNode replacingNode = geneticOperator.chooseNodeIn(potentialReplacingNodes, adiProxy.getReturnType());
			
			if(replacingNode != null)
				replacingNode = replacingNode.copy();
			else
				replacingNode = geneticOperator.findExternalNode(adiProxy.getReturnType());
			
			if(adiProxy != root)
				adiProxy.getParent().replaceArg(adiProxy, replacingNode);
			else
				module.replaceBranchRoot(adiProxy, replacingNode);
		}
		
		return !adiProxiesToBeReplaced.isEmpty();
	}
	
	@Override
	public final ADIDeletion copy() {
		return new ADIDeletion();
	}
}
