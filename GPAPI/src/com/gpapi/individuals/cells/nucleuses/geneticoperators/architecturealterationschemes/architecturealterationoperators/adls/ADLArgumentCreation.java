...
public final class ADLArgumentCreation implements ArchitectureAlterationOperatorInterface {
	...
	public ADLArgumentCreation(int maxArgs){
		this.maxArgs = maxArgs;
	}
	public ADLArgumentCreation(){
	...
	public final boolean architectureAlteration(Nucleus nucleus) {
		...
		ADL adl = adls.get(ThreadLocalRandom.current().nextInt(adls.size()));
		AbstractNode adlRoot = geneticOperator.chooseBranchRoot(adl);
		
		List<AbstractType> externalArgsTypes = adl.getExternalArgsTypes();
		if(externalArgsTypes.size() >= maxArgs)
			return false;
		
		AbstractNode argSubtreeRoot = chooseArgSubtreeRoot(adlRoot, externalArgsTypes.size(), geneticOperator);
		...
		if(argSubtreeRoot != adlRoot)
			argSubtreeRoot.getParent().replaceArg(argSubtreeRoot, newArgProxy);
		else
			adl.replaceBranchRoot(argSubtreeRoot, newArgProxy);
		adl.consolidate();
				...
				for(AbstractNode node : branchNodes){
					if(node instanceof ADLProxy){
						...
						if(adlProxy.getId() == adl.getId()){
							adaptAdlCall(adlProxy, argSubtreeRoot, geneticOperator);
							consolidate = true;
						}
					}
				}
	...
	private final AbstractNode chooseArgSubtreeRoot(AbstractNode adlRoot, int externalArgsSize, GeneticOperatorInterface geneticOperator){
		List<AbstractNode> selectableNodesList = geneticOperator.getDescendantNodes(adlRoot);
		HashSet<AbstractNode> selectableNodesSet = new HashSet<AbstractNode>(selectableNodesList);
		
		for(AbstractNode node : selectableNodesList){
			if(node instanceof ArgumentProxy){
				ArgumentProxy argProxy = (ArgumentProxy) node;
				if(argProxy.getId() >= externalArgsSize){
		...
		if(!selectableNodesSet.isEmpty()){
			selectableNodesList = new ArrayList<AbstractNode>(selectableNodesSet);
			return selectableNodesList.get(ThreadLocalRandom.current().nextInt(selectableNodesList.size()));
		} else
			return null;
	}
	private final void adaptAdlCall(ADLProxy adlProxy, AbstractNode originalArgSubtree, GeneticOperatorInterface geneticOperator){
		...
		for(AbstractNode leaf : geneticOperator.getDescendantNodes(newArgSubtree, true)){
			if(leaf instanceof ArgumentProxy){
				...
				AbstractNode replacingSubtree = geneticOperator.replicateSubtree(adlProxy.getArgs().get(argProxy.getId()));
	...
	@Override
	public final ADLArgumentCreation copy() {
		return new ADLArgumentCreation(maxArgs);
	}
}
