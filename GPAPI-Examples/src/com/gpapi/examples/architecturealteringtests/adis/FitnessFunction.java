...
public final class FitnessFunction implements GlobalFitnessFunctionInterface {
	...
	public FitnessFunction(double maxValue, int nValues, int nRounds){
		this(maxValue, nValues, nRounds, Integer.MAX_VALUE);
	}
		...
		for(EvolvedIndividual individual : population){
			...
			if(individual.getEggCell().getNucleus().getMaxModuleDepth() <= maxGenomeDepth){
				...
				for(ArrayList<List<AbstractType>> series : seriesList){
					...
					List<AbstractType> result = individual.executeUnconditionallySameArgs(Arrays.asList(RealValue.create(series.size())), Arrays.asList(series));
					
					RealValue average = (RealValue) result.get(0);
					RealValue product = (RealValue) result.get(1);
					
					if(average != null && product != null){
						error += Math.abs(average(series) - average.getValue()) / average(series);
	...
	private final double average(ArrayList<List<AbstractType>> series){
		double mean = 0.0;
		for(List<AbstractType> currentFields : series)
			mean += ((RealValue) currentFields.get(0)).getValue();
		return mean / series.size();
	}
