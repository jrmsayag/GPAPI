...
public final class FitnessFunction implements GlobalFitnessFunctionInterface {
			...
			if(individual.getEggCell().getNucleus().getMaxModuleDepth() <= maxGenomeDepth){
				...
				for(ArrayList<List<AbstractType>> series : seriesList){
					...
					RealValue result = (RealValue) individual.executeUnconditionally(
							0, 
							Arrays.asList(RealValue.create(series.size())), 
							Arrays.asList(series));
					if(result != null)
						error += Math.abs(targetFunction(series) - result.getValue());
					else
	...
	private final double targetFunction(ArrayList<List<AbstractType>> series){
		double mean = 0.0;
