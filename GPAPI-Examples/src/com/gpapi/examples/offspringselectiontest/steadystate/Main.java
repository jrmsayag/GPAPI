...
import com.gpapi.algorithm.SteadyStateLocalFitnessAlgorithm;
import com.gpapi.algorithm.SteadyStateLocalFitnessAlgorithmBuilder;
...
public class Main {
	...
	public static void main(String[] args) throws InvocationTargetException, InterruptedException, ExecutionException, IOException {
		...
		MaxSelectionPressureReached stagnationDetector = new MaxSelectionPressureReached(5, offspringBreedingOperator);
		
		SteadyStateLocalFitnessAlgorithm algorithm = SteadyStateLocalFitnessAlgorithmBuilder.create(
					new FitnessFunction(), 
					15000000, 
				...
				.setBreedingOperatorAndRemovalSelectionStrategy(offspringBreedingOperator, new Tournament(5, false, true))
				.setGeneralPurposeOperator(new Recycling(0, 1000, stagnationDetector))
				.build();
		
		algorithm.addObserver(stagnationDetector);
		
		algorithm.getView().setRefreshPeriod(100);
