...
package com.gpapi.examples.offspringselectiontest.generationallocal;
...
import com.gpapi.algorithm.GenerationalLocalFitnessAlgorithm;
import com.gpapi.algorithm.GenerationalLocalFitnessAlgorithmBuilder;
...
import com.gpapi.algorithm.stagnationdetectors.MaxSelectionPressureReached;
...
public class Main {
	...
	public static void main(String[] args) throws InvocationTargetException, InterruptedException, ExecutionException, IOException {
		...
		OffspringSelection offspringBreedingOperator = new OffspringSelection(new Tournament(5), 0, 0.75, 1, 0.75, 50);
		MaxSelectionPressureReached stagnationDetector = new MaxSelectionPressureReached(3, offspringBreedingOperator);
		
		GenerationalLocalFitnessAlgorithm algorithm = GenerationalLocalFitnessAlgorithmBuilder.create(
					new FitnessFunction(), 
					1500, 
					...
					4)
				.setNThreads(8)
				.setRecordingFrequency(0)
				.setBreedingOperator(offspringBreedingOperator)
				.setGeneralPurposeOperator(
						new KillOldest(5, 
								new Recycling(3, 5, stagnationDetector, 
										new Colonization(0.5, 5, new Random(false), new Random(false), stagnationDetector))))
				.build();
		
		algorithm.addObserver(stagnationDetector);
		
		algorithm.getView().setMaxGenerationsDisplayed(20);
		...
		SwingUtilities.invokeAndWait(() ->
			{
				...
				mainFrame.setTitle("Offspring selection tests (Symbolic regression)");
