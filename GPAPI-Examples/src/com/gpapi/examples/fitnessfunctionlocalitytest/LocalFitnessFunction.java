...
package com.gpapi.examples.fitnessfunctionlocalitytest;
...
public final class LocalFitnessFunction implements LocalFitnessFunctionInterface {
	...
	public LocalFitnessFunction(int maxBranchDepth){
	...
	public LocalFitnessFunction(){
		this(Integer.MAX_VALUE);
	}
	
	
	@Override
	public final void applyTo(EvolvedIndividual individual, int generation) {
		...
		if(individual.getEggCell().getNucleus().getMaxModuleDepth() <= maxBranchDepth){
			...
			for(Double currentX : xValues){
				...
				error += Math.abs(targetValue - indivValue);
			}
			individual.setRawFitness(error);
		}
		
		/*if(generation > 250){
			modulateWithExecutionCost(individual, 1.0E-10);
			modulateWithGenomeSize(individual, 1.0E-15);
		}*/
	}
