	...
	public static void main(String[] args) throws IOException, InvocationTargetException, InterruptedException, ExecutionException {
		...
		GlobalFitnessAlgorithm algorithm = GlobalFitnessAlgorithmBuilder.create(
					...
					EvolvedIndividualBuilder.create(geneticOperator)
						.setInitRpbsSize(25)
						.setRpbsTypes(Arrays.asList(RealValue.create(), RealValue.create(), RealValue.create()))
						.setRpbsArgsTypes(Arrays.asList(Sample.argsTypes, Sample.argsTypes, Sample.argsTypes))
						.setRpbsArgsNames(Arrays.asList(Sample.argsNames, Sample.argsNames, Sample.argsNames))
						.setInitAdfsSize(5)
						...
						.setAdfsArgsTypes(Arrays.asList(
									...
									Arrays.asList(RealValue.create(), RealValue.create(), RealValue.create(), RealValue.create()))).build(),
					500, 
					6)
				.setNThreads(6)
				.setBreedingOperator(new ParentSelection(new Tournament(7), 25, 0.5, -1))
				.setGeneralPurposeOperator(new Decimation(0.99, new KillOldest(5)))
				.build();
		...
		SwingUtilities.invokeAndWait(() ->
			{
				...
				mainFrame.setTitle("Shots success predictor");
