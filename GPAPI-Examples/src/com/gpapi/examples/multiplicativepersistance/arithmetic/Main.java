...
public class Main {
	...
	public static void main(String[] args) throws InvocationTargetException, InterruptedException, ExecutionException, IOException {
		ArrayList<AbstractNode> globalExternalNodes = new ArrayList<AbstractNode>();
			...
			globalExternalNodes.add(new Constant(RealValue.createWithIncrement(10000)));
			globalExternalNodes.add(new Constant(RealValue.createWithIncrement(10000000)));
		...
		GlobalFitnessAlgorithm algorithm = GlobalFitnessAlgorithmBuilder.create(
					...
					EvolvedIndividualBuilder.create(geneticOperator)
						...
						/*.setInitAdrsSize(5)
						.setAdrsTypes(Arrays.asList(RealValue.create(), RealValue.create()))
						.setAdrsArgsTypes(Arrays.asList(
								Arrays.asList(RealValue.create()),
								Arrays.asList(RealValue.create(), RealValue.create())))
						.setAdrsMaxCalls(Arrays.asList(2, 2))
						.setAdrsMaxRecursionDepths(Arrays.asList(10, 10))*/
						.build(), 
					2500, 
					16)
				.setNThreads(16)
		...
		DataManager.saveIndividual(bestGeneration.getBestIndividualFor(bestGeneration.getBestPopulation()), "multiplicativepersistance/best_arithmetic.ser");
	}
}
